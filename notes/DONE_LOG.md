...
3.8.2 password-recovery implementation milestone: completed the working Supabase recovery link, verified a production password change and subsequent Sign in with the new password, stabilized Sign in validation and recovery feedback, removed recovery resend, rejected authenticated users without company membership through the generic credentials path, added unique TokenHash subjects, and prepared a flat light Gmail-safe Coasterly AI email template. The owner paused the task after agreeing the complete Reset Password scenario contract. The final Supabase template application, Gmail iOS verification, and full production matrix remain pending, so this is not an accepted 3.8.2 checkpoint; 269 tests passed before documentation, сохранена 24.09
Backup `v3.8.2_password_recovery_chat_handoff` created and validated at 650,526 bytes with SHA-256 `3ad90ecdf3cd24f7cbd0606cee7d6cd5551860a1fad149e90da80d59233d0df6`, сохранена 24.09
