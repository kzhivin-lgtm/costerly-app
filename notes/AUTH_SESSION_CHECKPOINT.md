...
- User manually confirmed that refresh preserves authentication and that the
  accepted Upload geometry remains unchanged.
- Automated suite: 130 tests passed.
- Tests verify token restore/store behavior, `sessionStorage` rather than
  `localStorage`, and execution inside the hidden Sidebar context.
